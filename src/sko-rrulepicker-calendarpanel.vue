<template>
  <div class="sko-cal">
    <div class="sko-cal-header" v-if="currentPanel === 'time'">
      <a @click="currentPanel = 'date'">{{now.toLocaleDateString()}}</a>
    </div>
    <div class="sko-cal-header" v-else>
      <a class="sko-cal__prev-icon" @click="changeYear(-1)">&laquo;</a>
      <a v-show="currentPanel === 'date'" class="sko-cal__prev-icon" @click="changeMonth(-1)">&lsaquo;</a>
      <a class="sko-cal__next-icon" @click="changeYear(1)">&raquo;</a>
      <a v-show="currentPanel === 'date'" class="sko-cal__next-icon" @click="changeMonth(1)">&rsaquo;</a>
      <a @click="showMonths">{{months[currentMonth]}}</a>
      <a @click="showYears">{{currentYear}}</a>
    </div>
    <div class="sko-cal-content">
      <table class="sko-cal-table" v-show="currentPanel === 'date'">
        <thead>
          <tr>
      ...
      <div class="sko-cal-year" v-show="currentPanel === 'years'">
        <a v-for="year in years" @click="selectYear(year)" :class="{'current': currentYear === year}">{{year}}</a>
      </div>
      <div class="sko-cal-month" v-show="currentPanel === 'months'">
        <a v-for="(month, index) in months" @click="selectMonth(index)" :class="{'current': currentMonth === index}">{{month}}</a>
      </div>
      <div class="sko-cal-time"
        v-show="currentPanel === 'time'" >
        <div v-if="timeSelectOptions.length" class="sko-cal-time-list-wrapper">
          <ul class="sko-cal-time-list">
            <li class="sko-cal-time-item sko-cal-time-picker-item"
        ...
        <div v-else class="sko-cal-time-list-wrapper"
          :style="{width: 100 / times.length + '%' }"
          v-for="(time, index) in times"
          :key="index">
          <ul class="sko-cal-time-list">
            <li class="sko-cal-time-item"
              v-for="num in time"
...
<script>
  ...
  export default {
    ...
    data() {
      ...
      return {
        months: translation.months.map(item => item.value),
        dates: [],
        years: [],
        now: new Date(),
        currentPanel: 'date',
    ...
    computed: {
      days() {
        const days = this.$parent.translation.days.map(item => item.value)
        ...
        if (this.$parent.type === 'datetime') {
          ...
          this.$nextTick(() => {
            Array.prototype.forEach.call(
              this.$el.querySelectorAll('.sko-cal-time-list-wrapper'),
              (el) => {
